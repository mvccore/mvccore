...
interface IConstants {
	...
	/**
	 * Security token minimal validity time.
	 * Every token in cookie must be transfered 
	 * between server and client for this minimal time,
	 * doesn't matter if there is any form submit except
	 * user login/logout. After this time, token could 
	 * be changed in first form submit or after max time.
	 * @var int
	 */
	const SECURITY_TOKEN_EXPIRATION_MIN_TIME	= 15;

	/**
	 * Security token maximum validity time.
	 * When token is tranfered between server and client
	 * more than this time, it's automatically regenerated
	 * by any request type.
	 * @var int
	 */
	const SECURITY_TOKEN_EXPIRATION_MAX_TIME	= 60;

	/**
	 * The seconds after the last security token expired 
	 * for client queries launched with an old security 
	 * cookie token while a new security cookie token 
	 * has already been sent from the server.
	 * @var int
	 */
	const SECURITY_TOKEN_EXPIRATION_LAST_TIME	= 30;
